...
function AuthPage() {
  ...
  async function onContinueEmail(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await checkEmailExists({ data: { email: cleanEmail } });
      setStep(res?.exists ? "login" : "signup");
    } catch (err) {
      console.warn("Could not check email existence, proceeding to password step:", err);
      // Fallback seamlessly to login step with toggle available so user is never blocked
      setStep("login");
    } finally {
      setBusy(false);
    }
              ...
              {step === "login" ? (
                <>
                  <button
                    ...
                    disabled={busy}
                    className="font-medium text-muted-foreground transition-colors hover:text-foreground underline underline-offset-4"
                  >
                    Forgot password?
                  </button>
                  <button
                ...
                <button
                  ...
                  className="ml-auto font-semibold text-primary transition-opacity hover:opacity-80"
                >
