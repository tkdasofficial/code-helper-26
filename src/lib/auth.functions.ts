 ...
 */
export const checkEmailExists = createServerFn({ method: "POST", name: "checkEmailExists" })
...
(checkEmailExists as any).__name = "checkEmailExists";

export const recordEmailRegistered = createServerFn({ method: "POST", name: "recordEmailRegistered" })
  .inputValidator((data: unknown) => z.object({ email: z.string().email() }).parse(data))
  .handler(async ({ data }) => {
    try {
...
(recordEmailRegistered as any).__name = "recordEmailRegistered";
